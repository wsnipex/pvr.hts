...
def buildPlugin(Map addonParams = [:])
{
	...
	def addon = "pvr.hts"
	def distlist = []
	UBUNTU_DISTS.keySet().each{ k -> distlist.add(UBUNTU_DISTS[k].join(',')) }
	def all_dists = distlist.join(',').tokenize(',').unique().join(',')
	...
	properties([
		...
		parameters([
			...
			extendedChoice('dists', all_dists, all_dists, 'Ubuntu version to build for'),
		...
		tasks[platform] = {
			throttle(["binary-addons/${platform}-${version}"])
			{
				node(PLATFORMS_VALID[platform])
				{
					...
					{
						def packageversion
						def dists = UBUNTU_DISTS[PPA_VERSION_MAP[version]]
